...
public class DvbSubtitlesParser {
    ...
    /* Constants */
    private static final int DVBSUB_MAX_OBJECTS_STORED = 16;
    ...
    private class DisplayDefinition {
        ...
        int flags;
        int displayWindowHorizontalPositionMinimum;
        int displayWindowHorizontalPositionMaximum;
        int displayWindowVerticalPositionMinimum;
        int displayWindowVerticalPositionMaximum;
        ...
        DisplayDefinition() {
            this.displayWidth = 719;
            this.displayHeight = 575;
            this.displayWindowHorizontalPositionMinimum = 0;
            this.displayWindowHorizontalPositionMaximum = 719;
            this.displayWindowVerticalPositionMinimum = 0;
            this.displayWindowVerticalPositionMaximum = 575;
            paintObject = new Paint();
            updateBitmapResolution();
            canvasObject = new Canvas(bitmaps[currentBitmap]);
            lastValidBitmap = this.bitmaps[currentBitmap];
        }

        void updateBitmapResolution() {
            for (int i = 0; i < NUM_BITMAPS; i++) {
              this.bitmaps[i] = Bitmap.createBitmap(this.displayWidth + 1, this.displayHeight + 1,
    ...
    */
    ...
    DvbSubtitlesParser(int subtitlePage, int ancillaryPage) {
        ...
        subtitleService.displayDefinition = new DisplayDefinition();
    ...
    private DisplayDefinition parseDisplayDefinitionSegment() {
        ...
        */
        ...
        display.updateBitmapResolution();
        ...
        */
        ...
        while (!endOfPixelCodeString) {
            ...
            if (runLength != 0) {
                ...
                paintInCanvas(column, line, runLength, lineHeigth, colour);
    ...
    private void paintInCanvas(int column, int line, int runLength, int lineHeigth, int colour) {
        subtitleService.displayDefinition.paintObject.setColor(colour);
        subtitleService.displayDefinition.paintObject.setStyle(Paint.Style.FILL_AND_STROKE);
        subtitleService.displayDefinition.paintObject.setPathEffect(null);
        subtitleService.displayDefinition.canvasObject.drawRect(
                column, line, column + runLength, line + lineHeigth, subtitleService.displayDefinition.paintObject);
    }
        ...
        if (sync == 0xff || tsStream.bitsLeft() == 0) {                   // end_of_PES_data_field_marker
            ...
            if (subtitleService.pageComposition != null) {
                ...
                for (a = 0; a < subtitleService.pageComposition.pageRegions.size(); a++) {
                    ...
                    subtitleService.displayDefinition.canvasObject.clipRect(
                    ...
                    if ((regionComposition.flags & REGION_FILL_FLAG )!= 0) { // TODO: verify
                        int colour;
                        ...
                        subtitleService.displayDefinition.paintObject.setColor(colour);
                        subtitleService.displayDefinition.paintObject.setStyle(Paint.Style.FILL);
                        subtitleService.displayDefinition.paintObject.setPathEffect(null);
                        subtitleService.displayDefinition.canvasObject.drawRect(
                                baseHorizontalAddress, baseVerticalAddress,
                ...
                subtitleService.displayDefinition.canvasObject =
                        new Canvas(subtitleService.displayDefinition.bitmaps[subtitleService.displayDefinition.currentBitmap]);
                        subtitleService.displayDefinition.canvasObject.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
                return subtitleService.displayDefinition.lastValidBitmap;
            }
        } else {
